...
module "base_transitivity" {
  ...
  regions             = keys(local.net_hub_subnet_primary_ranges)
  vpc_name            = module.net_hub_shared_vpc[0].network_name
  gw_subnets          = { for region in keys(local.net_hub_subnet_primary_ranges) : region => "sb-c-shared-hub-${region}" }
  regional_aggregates = local.net_hub_regional_aggregates
  commands = [
    ...
    "iptables -A FORWARD -s ${join(",", flatten(values(local.net_hub_regional_aggregates)))} -d ${join(",", flatten(values(local.net_hub_regional_aggregates)))} -j ACCEPT",
    # Drop everything else
  ...
  depends_on = [module.net_hub_shared_vpc]
}